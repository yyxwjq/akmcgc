...
from .util_funcs import unsorted_segment_sum
from .core import MLP

try:
    from ..utils.graph import get_edge_target_positions, get_pbc_distances
...
class LEFTNet(torch.nn.Module):
    ...
    def assemble_nodemask(edge_index: Tensor, pos: Tensor):
        node_mask = torch.zeros(pos.size(0), dtype=pos.dtype, device=pos.device)
        node_mask[:] = -1
        _i, _j = edge_index
        _ind = 0
        for center in range(pos.size(0)):
            ...
            _connected = torch.concat(
                [
                    _connected,
                    torch.tensor([center], dtype=_j.dtype, device=pos.device),
                ]
            )
    ...
    def forward(
        ...
        if cell is not None and pbc is not None:
            ...
            edge_vectors = out["distance_vec"]
            # Create artificial coord_diff and coord_cross for frame calculation
            # We're using the raw edge_vectors (not normalized) for frame construction
            coord_diff = edge_vectors
            ...
            coord_cross = torch.cross(pos_frame[edge_index[0]], target_pos, dim=1)
            norm = torch.sqrt(torch.sum(coord_cross**2, 1, keepdim=True)) + EPS
            coord_cross = coord_cross / norm
            coord_vertical = torch.cross(coord_diff, coord_cross, dim=1)
            dist = torch.linalg.norm(edge_vectors, dim=1)
        else:
