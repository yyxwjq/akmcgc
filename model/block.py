...
class EquivariantBlock(nn.Module):
    ...
    def forward(
        ...
        if cell is not None and pbc is not None:
            ...
            coord_cross = torch.cross(pos[edge_index[0]], target_pos, dim=1)
            coord_cross = coord_cross / (
                torch.linalg.norm(coord_cross, dim=1, keepdim=True) + self.norm_constant
            )
            radial = distances + 1e-8
            coord_diff = edge_vectors / (radial + self.norm_constant)
        else:
